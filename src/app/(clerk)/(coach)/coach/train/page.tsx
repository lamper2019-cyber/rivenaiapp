...
/**
 * RIVEN's training board — the coach's own push/pull/legs tracker.
 * Three days, one on screen at a time, each lift carrying its sets / reps /
 * weight and how long it's been since the weight moved.
 */
export default async function TrainPage() {
  const { userId: clerkId } = auth();

  // The (coach) layout already gates on role — this is just to resolve the
  // internal user id the board is keyed on.
  const user = clerkId
